...
# 自定义模块导入
from utils.extra_modules import seg_img, judge_seg
...
class Conv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):  # c_in、c_out、kernel_size、stride
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        # 未指定激活函数，使用默认silu，否则使用指定act参数，如果act不存在则不经过激活函数
...
class DetectMultiBackend(nn.Module):
    ...
    # 模型工作函数
    def forward(self, im, augment=False, visualize=False, chunking=False, seg=6):
        ...
        if self.pt:  # PyTorch
            ...
            if chunking:
                result_c1 = [judge_seg(y)]
                imgs_c4 = seg_img(im, c=seg, stride=self.stride)
                imgs_c2 = seg_img(im, c=2, stride=self.stride)
                result_c4 = np.array([[None for _ in range(seg)] for _ in range(seg)])
                result_c2 = np.array([[None for _ in range(2)] for _ in range(2)])
                for i in range(seg):
                    for j in range(seg):
                        a = imgs_c4[i][j]
                        r = self.model(a, augment, visualize) if augment or visualize else self.model(a)
                        result_c4[i][j] = judge_seg(r)
                for i in range(2):
                    for j in range(2):
                        a = imgs_c2[i][j]
                        r = self.model(a, augment, visualize)if augment or visualize else self.model(a)
                        result_c2[i][j] = judge_seg(r)
        ...
        if isinstance(y, (list, tuple)):
            result = self.from_numpy(y[0]) if len(y) == 1 else [self.from_numpy(x) for x in y]
        else:
            result = self.from_numpy(y)
...
# 已添加：SE、C3SE、C3_SE、CBAM、C3CBAM、C3_CBAM、CA、C3CA、C3_CA、ECA、C3ECA、C3_ECA
